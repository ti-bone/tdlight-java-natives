...
cmake \
  ...
  -DCMAKE_CXX_COMPILER_LAUNCHER="$CCACHE" \
  -DOPENSSL_USE_STATIC_LIBS=True \
  -DTD_GENERATED_BINARIES_DIR="$(realpath ../../implementations/tdlight/td_tools_build/td/generate)" \
  -DTD_SRC_DIR="$(realpath ../../implementations/tdlight)" \
  -DTD_ENABLE_LTO=ON \
  -DTDNATIVES_BIN_DIR="$(realpath ../tdjni_bin/)" \
  -DTDNATIVES_DOCS_BIN_DIR="$(realpath ../tdjni_docs/)" \
  -DTd_DIR:PATH="$(realpath ../../implementations/tdlight/build/td_bin/lib/cmake/Td)" \
  -DJAVA_SRC_DIR="$(realpath ../src/main/java)" \
  -DTDNATIVES_CPP_SRC_DIR="$(realpath ../src/main/cpp)" \
  ../src/main/cpp
cmake --build . --target install --config Release --parallel "$(nproc)"
cd ..
